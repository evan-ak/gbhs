...
class _DenseLayer(nn.Module):
    def __init__(self, num_input_features, growth_rate, bn_size, drop_rate, memory_efficient=False, turnoff_bn=False):
        super(_DenseLayer, self).__init__()
        self.turnoff_bn = turnoff_bn
        if not self.turnoff_bn :
...
class _DenseBlock(nn.ModuleDict):
    ...
    def __init__(self, num_layers, num_input_features, bn_size, growth_rate, drop_rate, memory_efficient=False, turnoff_bn=False):
...
class DenseNet(torch.nn.Module):
  ...
  def __init__(self, growth_rate=12, block_config=(6, 6, 6),
    ...
    self.features = torch.nn.Sequential(OrderedDict([
      ('conv0', torch.nn.Conv2d(6, num_init_features, kernel_size=7, stride=2,
...
class DenseNetSP(torch.nn.Module):
  r"""Densenet-BC model class, based on
  `"Densely Connected Convolutional Networks" <https://arxiv.org/pdf/1608.06993.pdf>`_
  ...
  def __init__(self, growth_rate=12, block_config=(6, 6, 6),
    ...
    super(DenseNetSP, self).__init__()
    ...
    for i, num_layers in enumerate(block_config):
      if i == self.branch_to :
        self.features, self.block1 = self.block2, self.features
        num_features *= 2
      block = _DenseBlock(
        num_layers=num_layers,
        num_input_features=num_features,
  ...
  def forward(self, x, y):
    # features = self.features(x)
    features_x = self.block1(x)
    features_y = self.block1(y)
